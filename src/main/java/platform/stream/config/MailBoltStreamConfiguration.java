...
@ConditionalOnProperty(name="spring.cloud.stream.bindings.mailBoltChannel.enabled111", havingValue="true",matchIfMissing=false)
public class MailBoltStreamConfiguration {
	...
	@StreamListener(value = MessageSink.MAIL_BOLT_CHANNEL)
	public void processMailBoltMessage(Message message) {
		try {
